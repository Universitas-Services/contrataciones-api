...
export class ManualesController {
    ...
    @ApiOperation({ summary: 'Generar manual', description: 'Genera un manual DOCX para el Ente del usuario autenticado' })
    @ApiResponse({ status: 201, description: 'Manual generado exitosamente' })
    @ApiResponse({ status: 403, description: 'No autorizado (solo ADMIN_ENTE y EJECUTOR)' })
    ...
    @Get()
    @ApiOperation({ summary: 'Listar manuales', description: 'Obtiene todos los manuales del Ente o de los Entes asignados (SUPERVISOR)' })
    ...
    @Get(':id')
    @ApiOperation({ summary: 'Ver manual', description: 'Obtiene los detalles de un manual específico' })
    ...
    @Get(':id/download')
    @ApiOperation({ summary: 'Descargar manual', description: 'Obtiene la URL de descarga del archivo DOCX' })
    @ApiParam({ name: 'id', description: 'ID del manual' })
