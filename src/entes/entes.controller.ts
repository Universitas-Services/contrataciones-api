...
export class EntesController {
    ...
    @Post()
    @Roles('UNIVERSITAS')
    @ApiOperation({ summary: 'Crear Ente', description: 'Crea un nuevo Ente Público con su usuario administrador' })
    ...
    create(
        @Body() createEnteDto: CreateEnteDto,
    ...
    @Get()
    @ApiOperation({ summary: 'Listar Entes', description: 'UNIVERSITAS ve todos, SUPERVISOR ve asignados, otros ven solo el suyo' })
    ...
    @Get(':id')
    @ApiOperation({ summary: 'Ver Ente', description: 'Obtiene detalles de un Ente específico' })
    ...
    @Delete(':id')
    @Roles('UNIVERSITAS')
    @ApiOperation({ summary: 'Eliminar Ente', description: 'Elimina un Ente (soft delete)' })
    @ApiParam({ name: 'id', description: 'ID del Ente' })
