...
describe('SupervisoresService', () => {
    ...
    describe('create', () => {
        it('debe crear un supervisor con Entes asignados', async () => {
            ...
            mockPrismaService.supervisorAsignacion.createMany.mockResolvedValue({ count: 2 });
            ...
            expect(mockPrismaService.supervisorAsignacion.createMany).toHaveBeenCalledWith({
                data: expect.arrayContaining([
                    expect.objectContaining({ enteId: 'ente-1', supervisorId: mockSupervisor.id }),
                    expect.objectContaining({ enteId: 'ente-2', supervisorId: mockSupervisor.id }),
                ]),
            });
        });
        ...
        it('debe lanzar ConflictException si email ya existe', async () => {
            ...
            mockPrismaService.usuario.findUnique.mockResolvedValue({ id: 'existing-user' });

            await expect(service.create(createDto, 'admin-id')).rejects.toThrow(ConflictException);
            await expect(service.create(createDto, 'admin-id')).rejects.toThrow('El email ya está registrado');
        });
    });
    ...
    describe('asignarEntes', () => {
        it('debe agregar y remover Entes correctamente', async () => {
            ...
            mockPrismaService.usuario.findUnique.mockResolvedValue(mockSupervisor);
            mockPrismaService.supervisorAsignacion.deleteMany.mockResolvedValue({ count: 1 });
            mockPrismaService.supervisorAsignacion.createMany.mockResolvedValue({ count: 2 });
