...
export class CreateSupervisorDto {
    ...
    @ApiProperty({ example: 'carlos.ramirez@supervision.gob.ve', description: 'Email único del supervisor' })
    @IsEmail()
    email: string;

    @ApiProperty({ example: 'supervisor123', description: 'Contraseña (mínimo 6 caracteres)', minLength: 6 })
    @IsString()
    @MinLength(6)
    password: string;
